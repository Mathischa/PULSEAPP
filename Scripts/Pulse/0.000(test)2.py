...
#===================Page Prédiction IA===================
    ...
    def _ia_build_prediction_page(self, df):
        ...
        def _redraw_confusion_matrix_graph():
            """Redessine la matrice de confusion du classifieur sur VALID."""
            ...
            # Prévision ensemble classification
            ...
            fig, ax = plt.subplots(figsize=(7.2, 5.2), facecolor="#00122e", constrained_layout=True)
            ax.set_facecolor("#00122e")
            ...
            classes = ["Prédit 0", "Prédit 1"]
            rows = ["Réel 0", "Réel 1"]

            ax.set_xticks(np.arange(2))
            ax.set_yticks(np.arange(2))
            ax.set_xticklabels(classes, color="white", fontsize=11)
            ax.set_yticklabels(rows, color="white", fontsize=11)
            ...
            ax.set_title(f"Matrice de confusion – {filiale}{year_txt}", color="white", fontsize=14)
            ...
            # Petit résumé KPI dans la figure
            kpi_txt = (
                f"Accuracy: {acc:.1%}   |   "
        ...
        def _train_trafic_voyageurs_v33():
            ...
            Objectif :
            - Prévision centrale extrêmement robuste grâce à l'incorporation d'XGBoost et de la volatilité
            - Blending pondéré de manière plus agressive sur la performance
            - Bandes calibrées avec alert ~5%
            - MC weekday-block conditionnel
            """
            ...
            try:
                ...
                # ======================================================================
                # 2) Série métier + features (Amélioré avec Volatilité)
                # ======================================================================
                ...
                # NOUVEAU: Features de Volatilité (très puissant pour les modèles d'arbres)
                ...
                base_features = [
                    ...
                    "roll_std_prev_7", "roll_std_prev_30",  # <- New!
                ...
                # ======================================================================
                # 6) Models : ensemble hétérogène (CatBoost + LightGBM + XGBoost + ET)
                # ======================================================================
                try:
                ...
                # Pondération Softmax plus agressive pour récompenser le meilleur modèle
                ...
                # ----- CatBoost CLS -----
                cls_cat_base = CatBoostClassifier(random_seed=42, verbose=0, loss_function="Logloss", eval_metric="AUC", auto_class_weights="Balanced")
                cls_cat_params = {"depth": randint(4, 9), "learning_rate": uniform(0.01, 0.08), "iterations": randint(350, 1000), "l2_leaf_reg": uniform(1.0, 8.0)}
                ...
                # ----- LightGBM CLS -----
                cls_lgb_base = LGBMClassifier(random_state=42, objective="binary", verbosity=-1)
                cls_lgb_params = {"n_estimators": randint(250, 800), "num_leaves": randint(15, 63), "max_depth": randint(3, 8), "learning_rate": uniform(0.01, 0.1)}
                ...
                # ----- XGBoost CLS (NOUVEAU) -----
                if HAS_XGB:
                    cls_xgb_base = XGBClassifier(random_state=42, eval_metric="auc", use_label_encoder=False, verbosity=0)
                    cls_xgb_params = {"n_estimators": randint(200, 700), "max_depth": randint(3, 8), "learning_rate": uniform(0.01, 0.1), "subsample": uniform(0.6, 0.4)}
                ...
                # ----- ExtraTrees CLS -----
                cls_et = ExtraTreesClassifier(n_estimators=800, max_depth=15, min_samples_leaf=2, class_weight="balanced", random_state=42, n_jobs=-1)
                ...
                # CatBoost REG
                reg_cat_base = CatBoostRegressor(random_seed=42, verbose=0, loss_function="RMSE")
                reg_cat_params = {"depth": randint(4, 9), "learning_rate": uniform(0.01, 0.08), "iterations": randint(400, 1200)}
                ...
                # LightGBM REG
                reg_lgb_base = LGBMRegressor(random_state=42, objective="regression", verbosity=-1)
                reg_lgb_params = {"n_estimators": randint(300, 1000), "num_leaves": randint(15, 63), "max_depth": randint(3, 8), "learning_rate": uniform(0.01, 0.08)}
                ...
                # XGBoost REG (NOUVEAU)
                if HAS_XGB:
                    reg_xgb_base = XGBRegressor(random_state=42, objective="reg:squarederror", verbosity=0)
                    reg_xgb_params = {"n_estimators": randint(250, 800), "max_depth": randint(3, 8), "learning_rate": uniform(0.01, 0.08), "subsample": uniform(0.6, 0.4)}
                ...
                # ExtraTrees REG
                reg_et = ExtraTreesRegressor(n_estimators=1000, max_depth=15, min_samples_leaf=2, random_state=42, n_jobs=-1)
                ...
                for name, mdl in zip(reg_model_names, reg_models):
                    ...
                    reg_scores.append(1.0 / max(rmse_i ** 2, 1e-6))  # Score d'autant plus fort que l'erreur est faible
                ...
                if len(idx_corr) < 60:
                    ...
                    def _fit_corr(y_corr, label):
                        ...
                        # CatBoost
                        mdl_cat = CatBoostRegressor(iterations=600, learning_rate=0.04, depth=6, random_seed=42, verbose=0)
                        mdl_cat.fit(X_corr_train, y_corr, sample_weight=w_corr)
                        corr_models_local.append(mdl_cat)
                        corr_names_local.append("CatBoost")

                        # LightGBM
                        mdl_lgb = LGBMRegressor(n_estimators=400, learning_rate=0.04, num_leaves=31, random_state=42, verbosity=-1)
                        ...
                        # XGBoost
                        if HAS_XGB:
                            mdl_xgb = XGBRegressor(n_estimators=300, learning_rate=0.04, max_depth=5, random_state=42, verbosity=0)
                ...
                if denom > 1e-9:
                    ...
                    scale = max(0.85, min(scale, 1.15)) # Moins violent que 0.7-1.3 avec un ensemble meilleur
                else:
                    scale = 1.0
                ...
                def _kpi(parent, title, value, subtitle, col):
                    ...
                    card.grid(row=0, column=col, sticky="nsew", padx=10, pady=10)
                    ctk.CTkLabel(card, text=title, font=("Segoe UI", 12), text_color="#9fb7dd").grid(row=0, column=0, sticky="w", padx=14, pady=(12, 0))
                    ctk.CTkLabel(card, text=value, font=("Segoe UI Semibold", 22, "bold"), text_color="white").grid(row=1, column=0, sticky="w", padx=14, pady=(2, 6))
                    ctk.CTkLabel(card, text=subtitle, font=("Segoe UI", 11), text_color="#7ea2d8").grid(row=2, column=0, sticky="w", padx=14, pady=(0, 12))

                _kpi(kpi_frame, "MAE", f"{mae:.0f}", "Erreur moyenne", 0)
                _kpi(kpi_frame, "RMSE", f"{rmse:.0f}", "Risque pics", 1)
                _kpi(kpi_frame, "R²", f"{r2:.3f}", "Pouvoir expl.", 2)
                _kpi(kpi_frame, "Cov95 raw", f"{coverage_95:.1%}", "P05–P95 calib", 3)
                _kpi(kpi_frame, "Alert raw", f"{alert_95:.1%}", "1-Cov raw", 4)
                _kpi(kpi_frame, "Width raw", f"{width_95:.0f}", "Largeur raw", 5)
                _kpi(kpi_frame, "Cov95 conf", f"{coverage_c:.1%}", "Après conformal", 6)
                _kpi(kpi_frame, "Alert conf", f"{alert_c:.1%}", "objectif ~5%", 7)
                _kpi(kpi_frame, "Width conf", f"{width_c:.0f}", "Largeur conf", 8)
                _kpi(kpi_frame, "Scale", f"{scale:.3f}", "Ratio totals", 9)
                _kpi(kpi_frame, "Offset", f"{offset:.0f}", "Median resid", 10)
