...
#===================Page Accueil + fonctions de navigation===================
    def creer_accueil(self):
        ...
        try:
            ...
            fichiers, existing, nb_fichiers = [], [], 0
            derniere_maj, annees_dispos = "—", []
        ...
        def _rs():
            w = self.winfo_width()
            h = self.winfo_height()
            if w <= 1: w = max(1280, self.winfo_screenwidth()-140)
            if h <= 1: h = max(800,  self.winfo_screenheight()-140)
            tier = ("xl" if w>=1800 else "lg" if w>=1480 else
                    "md" if w>=1220 else "sm" if w>=980  else "xs")
            sb_auto = ("full" if w>=1200 else "compact" if w>=960 else "icon")
            return {
                "w":w, "h":h, "tier":tier, "sb_auto":sb_auto,
                "hdr_h":       88 if tier in("xl","lg") else 80 if tier=="md" else 72,
                "pad":         28 if tier in("xl","lg") else 22 if tier=="md" else 16 if tier=="sm" else 12,
                "gap":         14 if tier in("xl","lg") else 10 if tier=="md" else 8,
                "inner":       20 if tier in("xl","lg") else 16 if tier=="md" else 13 if tier=="sm" else 11,
                "hero_stack":  w<1380, "btn_stack": w<820,
                "kpi_cols":    3 if w>=1400 else 2 if w>=900 else 1,
                "act_cols":    3 if w>=1480 else 2 if w>=980 else 1,
                "met_cols":    3 if w>=1350 else 2 if w>=920 else 1,
                "hero_fs":     32 if tier=="xl" else 27 if tier=="lg" else 23 if tier=="md" else 20 if tier=="sm" else 18,
                "hero_sub_fs": 14 if tier in("xl","lg") else 13 if tier=="md" else 12,
                "sec_fs":      20 if tier in("xl","lg") else 17 if tier=="md" else 15 if tier=="sm" else 13,
                "card_fs":     12 if tier in("xl","lg") else 11,
                "card_val_fs": 32 if tier=="xl" else 28 if tier=="lg" else 24 if tier=="md" else 21 if tier=="sm" else 19,
                ...
                "hero_wrap":   max(340,min(820,w-580)),
                "detail_wrap": max(380,min(960,w-380)),
                "sum_w":       340 if w>=1580 else 300 if w>=1380 else 0,
                "hdr_sub":     w>=1040,
            }
        ...
        def _frame(parent, fg=None, radius=16, bw=1, bc=None, **kw):
            return ctk.CTkFrame(parent, fg_color=fg or C["surface"],
                                corner_radius=radius, border_width=bw,
                                border_color=bc or C["border"], **kw)
        ...
        def _btn(parent, text, cmd=None, h=38, w=0, radius=10,
            ...
            return ctk.CTkButton(parent, text=text, command=cmd, height=h, width=w,
        ...
        def _badge(parent, text, fg=None, tc=None, bc=None):
            pill = ctk.CTkFrame(parent, fg_color=fg or C["surface_3"],
                                corner_radius=999, border_width=1,
                                border_color=bc or C["border"])
            lbl = ctk.CTkLabel(pill, text=text, text_color=tc or C["text_2"],
        ...
        def _section_hdr(parent, title, subtitle=None):
            ...
            t = ctk.CTkLabel(wrap, text=title, text_color=C["text"],
                            font=(FONT_HEAD, 18, "bold"))
            t.grid(row=0, column=0, sticky="w")
            s = None
            if subtitle:
                s = ctk.CTkLabel(wrap, text=subtitle, text_color=C["muted"],
                                font=(FONT_BODY, 12))
        ...
        def _hover(widget, nfg, hfg):
            def _in(_=None):
                try: widget.configure(fg_color=hfg)
                except Exception: pass
            def _out(_=None):
                try: widget.configure(fg_color=nfg)
                except Exception: pass
        ...
        # =========================================================
        # TOOLTIP NATIF TKINTER
        # =========================================================
        _tt = {"job": None, "win": None}

        def _tt_show(widget, text):
            def _do():
                if _tt["win"] and _exists(_tt["win"]):
                    try: _tt["win"].destroy()
                    except Exception: pass
                if not _exists(widget): return
                tip = tk.Toplevel(self)
                tip.overrideredirect(True)
                tip.attributes("-topmost", True)
                tip.configure(bg=C["surface_3"])
                tk.Label(tip, text=text, bg=C["surface_3"], fg=C["text"],
                        font=(FONT_BODY, 11), padx=12, pady=7).pack()
                tip.update_idletasks()
                wx = widget.winfo_rootx() + widget.winfo_width() + 6
                wy = widget.winfo_rooty() + (widget.winfo_height()-tip.winfo_reqheight())//2
                tip.geometry(f"+{wx}+{wy}")
                _tt["win"] = tip
            if _tt["job"]:
                try: self.after_cancel(_tt["job"])
                except Exception: pass
            _tt["job"] = self.after(280, _do)
        ...
        def _render_detail():
            ...
            tone_c = {"neutral":C["primary_text"],"success":C["success"],
                    "warning":C["warning"]}.get(detail_state["tone"],C["primary_text"])
            pill,_ = _badge(body,"● Détail interactif",fg=C["surface_3"],tc=tone_c,bc=C["border"])
            pill.pack(anchor="w", pady=(0,14))
            for line in (detail_state["lines"] or ["Aucune donnée disponible."]):
                row = ctk.CTkFrame(body, fg_color="transparent")
                row.pack(fill="x", pady=(0,6))
                ctk.CTkLabel(row, text="›", width=14, text_color=C["primary"],
                            font=(FONT_HEAD,r["body_fs"]+1,"bold")).pack(
                    side="left", anchor="n", padx=(0,8))
                ctk.CTkLabel(row, text=line, text_color=C["text_2"],
                            font=(FONT_BODY,r["body_fs"]), justify="left",
                            wraplength=r["detail_wrap"]).pack(
                    side="left", fill="x", expand=True)

        def _set_detail(title, lines, tone="neutral"):
            detail_state.update(title=title,
                                lines=lines or ["Aucune donnée disponible."],
                                tone=tone)
            _render_detail()

        def _show_years():
            lines = [f"Années : {', '.join(str(a) for a in annees_dispos)}"] \
                if annees_dispos else ["Aucune année détectée."]
            _set_detail("Périmètre des fichiers mensuels", lines)

        def _show_filiales():
            _set_detail("Organisation des filiales",
                        list(filiales_list) or ["Aucune filiale configurée."], "success")

        def _show_overview():
            _set_detail("Résumé plateforme", [
        ...
        try:
            ...
            nh = 32; nw = int(nh*_img.width/max(_img.height,1))
            try:   _img = _img.resize((nw,nh), Image.Resampling.LANCZOS)
            except: _img = _img.resize((nw,nh), Image.ANTIALIAS)
            cimg = CTkImage(light_image=_img, dark_image=_img, size=(nw,nh))
            ll = ctk.CTkLabel(logo_wrap, image=cimg, text="")
            ll.image = cimg; ll.pack(side="left", pady=28)
        except Exception:
        ...
        hdr_sub = _lbl(title_wrap,"Plateforme d'analyse des flux de trésorerie",
                    C["muted"],(FONT_BODY,11))
        ...
        status_pill = ctk.CTkFrame(hdr_right, fg_color=C["success_dim"],
                                corner_radius=999, border_width=1, border_color=C["success"])
        ...
        quit_btn = ctk.CTkButton(hdr_right, text="Quitter",
                                command=self.demander_confirmation_quit,
                                height=36, width=88, corner_radius=8,
                                fg_color=C["danger"], hover_color=C["danger_hover"],
                                text_color="white", font=(FONT_HEAD,11,"bold"))
        ...
        # =========================================================
        # SIDEBAR — DYNAMIQUE ANIMÉE
        # =========================================================
        SB_FULL    = 272
        SB_COMPACT = 196
        SB_ICON    = 66
        SB_STEP_MS = 12    # ms entre frames d'animation
        SB_STEP_PX = 15    # px par frame

        _sb = {"mode": "full", "target": SB_FULL, "manual": False}
        ...
        burger_btn = ctk.CTkButton(
            burger_row, text="☰", width=38, height=32, corner_radius=8,
            fg_color=C["surface_3"], hover_color=C["surface_4"],
            text_color=C["text_2"], font=(FONT_BODY,14), anchor="center"
        )
        ...
        def _nav_item(parent, icon, label, short, cmd, active=False):
            ...
            frame = ctk.CTkFrame(parent, fg_color=nfg, corner_radius=10,
                                border_width=1, border_color=bc)
            frame.pack(fill="x", pady=2)
            ein, eout = _hover(frame, nfg, hfg)

            btn = ctk.CTkButton(frame, text=f"{icon}  {label}", command=cmd,
                                anchor="w", height=40, corner_radius=10,
                                fg_color="transparent", hover=False,
                                text_color=tc, font=(FONT_BODY,13))
            btn.pack(fill="x", padx=2, pady=2)
            btn.bind("<Enter>", ein); btn.bind("<Leave>", eout)

            # Tooltip pour mode icône
            for w_ in (frame, btn):
                w_.bind("<Enter>", lambda e, l=label: _tt_show(e.widget, l))
                w_.bind("<Leave>", _tt_hide)

            ref = {"frame":frame, "btn":btn, "icon":icon,
                "text":label, "short":short, "active":active}
            nav_items.append(ref)
            return ref
        ...
        # Info bas sidebar — dans un frame fixe hors du scroll
        ...
        # ── Engine d'animation sidebar ───────────────────────────
        def _sb_animate_to(target_w):
            _cancel_job("_sb_anim_job")
            _sb["target"] = target_w

            def _step():
                if not _exists(sidebar): return
                cur  = sidebar.cget("width")
                diff = target_w - cur
                if abs(diff) <= SB_STEP_PX:
                    sidebar.configure(width=target_w)
                    _sb_apply_content()
                    return
                sidebar.configure(width=cur + (SB_STEP_PX if diff>0 else -SB_STEP_PX))
                self._sb_anim_job = self.after(SB_STEP_MS, _step)

            self._sb_anim_job = self.after(SB_STEP_MS, _step)

        def _sb_apply_content():
            """Met à jour textes/layout sidebar selon le mode courant."""
            if not _exists(sidebar): return
            mode = _sb["mode"]
            r    = _rs_cache["v"]
            ...
            if mode == "icon":
                ...
                burger_btn.configure(width=42, anchor="center")
            elif mode == "compact":
                sb_nav_lbl.configure(text="")
                burger_btn.configure(width=38, anchor="center")
            else:
            ...
            for sec in sb_sections:
                if mode == "full":
                    sec["lbl"].configure(text=sec["text"],
                                        font=(FONT_HEAD, max(9,r["sm_fs"]),"bold"))
                elif mode == "compact":
                    # Abréviation
                    sec["lbl"].configure(text=sec["text"][:4].upper()+"…",
                                        font=(FONT_HEAD,8,"bold"))
                else:
            ...
            for item in nav_items:
                if mode == "icon":
                    t=item["icon"]; a="center"; fs=r["nav_fs"]+3
                    item["btn"].configure(text=t, anchor=a, height=r["btn_h"],
                                        font=(FONT_BODY,fs), width=46)
                elif mode == "compact":
                    t=f"{item['icon']}  {item['short']}"; a="w"; fs=r["nav_fs"]-1
                    item["btn"].configure(text=t, anchor=a, height=r["btn_h"],
                                        font=(FONT_BODY,fs), width=0)
                else:
                    t=f"{item['icon']}  {item['text']}"; a="w"; fs=r["nav_fs"]
                    item["btn"].configure(text=t, anchor=a, height=r["btn_h"],
                                        font=(FONT_BODY,fs), width=0)

            # Info card bas
            if mode != "icon":
                if not sb_info.winfo_manager(): sb_info.pack(fill="x")
                sb_info_v.configure(
                    text="Version produit" if mode=="full" else "Ver.",
                    font=(FONT_BODY, r["sm_fs"]))
                sb_info_ver.configure(
                    font=(FONT_HEAD, 16 if mode=="full" else 13, "bold"))
                sb_info_date.configure(
                    text=(f"MAJ : {derniere_maj}" if mode=="full" else derniere_maj[:10]),
                    font=(FONT_BODY, r["sm_fs"]))
            else:
                if sb_info.winfo_manager(): sb_info.pack_forget()

        def _sb_set(mode):
            if mode == _sb["mode"]: return
            _sb["mode"] = mode
            target = {"full":SB_FULL,"compact":SB_COMPACT,"icon":SB_ICON}[mode]
            _sb_animate_to(target)

        def _burger_toggle():
            _sb["manual"] = True   # l'utilisateur a agi manuellement
        ...
        hero_pill, hero_pill_lbl = _badge(
            left_hero,"Plateforme analytique  •  Finance & Pilotage",
            fg=C["primary_dim"], tc=C["primary_text"], bc=C["primary"])
        hero_pill.pack(anchor="w", pady=(0,16))

        hero_title = _lbl(left_hero,
                        "Transformez les flux de trésorerie\nen décisions fiables.",
                        C["text"],(FONT_HEAD,28,"bold"), justify="left")
        hero_title.pack(anchor="w")

        hero_desc = _lbl(left_hero,
                        ...
                        C["text_2"],(FONT_BODY,13), justify="left", wraplength=720)
        hero_desc.pack(anchor="w", pady=(12,22))

        hero_btns = ctk.CTkFrame(left_hero, fg_color="transparent")
        hero_btns.pack(anchor="w", fill="x")
        hero_primary = _primary_btn(hero_btns,"  Ouvrir la visualisation",
                                    cmd=self.creer_page_graphique, h=42, w=210)
        hero_ghost   = _btn(hero_btns,"  Importer des profils",
                            cmd=self.importer_les_profils, h=42, w=188)
        ...
        for mlbl,mval,mac in [
            ...
            met_refs.append({"card":mc,"lbl":ml,"val":mv,"accent":mac})
        ...
        def _mini(parent, lt, val, ac=None):
            ...
            inn = ctk.CTkFrame(mc, fg_color="transparent"); inn.pack(fill="x",padx=12,pady=10)
            ll = _lbl(inn,lt,C["muted"],(FONT_BODY,10)); ll.pack(anchor="w")
            vl = _lbl(inn,val,ac or C["text"],(FONT_HEAD,13,"bold")); vl.pack(anchor="w",pady=(2,0))
            mini_stats.append({"lbl":ll,"val":vl,"accent":ac})
        ...
        _lbl(sum_card,("Commencez par la visualisation graphique pour contrôler les écarts "
                    "réel / prévisions, puis poursuivez avec les analyses flux et détection."),
            C["text_2"],(FONT_BODY,11), justify="left", wraplength=280
            ).pack(anchor="w", padx=16, pady=(6,16))

        # =========================================================
        # KPI
        # =========================================================
        kpi_hdr = _section_hdr(main,"Indicateurs clés",
                            "Vue synthétique de l'état de la plateforme et du périmètre chargé.")
        kpi_hdr["wrap"].grid(row=1,column=0,sticky="ew",padx=24,pady=(4,4))
        kpi_cont = ctk.CTkFrame(main, fg_color="transparent")
        kpi_cont.grid(row=2,column=0,sticky="ew",padx=24,pady=(0,6))
        kpi_cards = []

        def _kpi(parent, icon, title, value, sub, detail_cb=None):
            nfg=C["surface_2"]; hfg=C["surface_3"]
            card = _frame(parent, fg=nfg, radius=16, bw=1, bc=C["border"])
            card.grid_columnconfigure(0, weight=1); _hover(card,nfg,hfg)
            top=ctk.CTkFrame(card,fg_color="transparent")
            top.grid(row=0,column=0,sticky="ew",padx=18,pady=(18,8))
            ip=ctk.CTkFrame(top,fg_color=C["surface_4"],corner_radius=8,
                            border_width=1,border_color=C["border"])
            ip.pack(side="left")
            _lbl(ip,icon,C["accent"],(FONT_BODY,14)).pack(padx=10,pady=6)
            tl=_lbl(card,title,C["muted"],(FONT_BODY,12))
            tl.grid(row=1,column=0,sticky="w",padx=18,pady=(0,2))
            vl=_lbl(card,value,C["text"],(FONT_HEAD,30,"bold"))
            vl.grid(row=2,column=0,sticky="w",padx=18)
            sl=_lbl(card,sub,C["muted_2"],(FONT_BODY,11),justify="left",wraplength=300)
            sl.grid(row=3,column=0,sticky="w",padx=18,pady=(6,0))
            btn=None
            if detail_cb:
                btn=_btn(card,"Voir le détail",cmd=detail_cb,h=30,w=130,font_size=11)
                btn.grid(row=4,column=0,sticky="w",padx=18,pady=(10,18))
                _click_tree(card,detail_cb)
            ref={"card":card,"icon_pill":ip,"title":tl,"val":vl,"sub":sl,"btn":btn}
            kpi_cards.append(ref); return ref

        _kpi(kpi_cont,"📁","Fichiers mensuels",
            f"{nb_fichiers:,}".replace(",", " "),
            "Historique consolidé des fichiers mensuels disponibles.",_show_years)
        _kpi(kpi_cont,"🏢","Filiales couvertes",
            f"{nb_filiales:,}".replace(",", " "),
            "Entités identifiées dans la configuration active.",_show_filiales)
        _kpi(kpi_cont,"🕒","Dernière actualisation",
            derniere_maj,"Calculée depuis les dates de modification.",_show_overview)

        # =========================================================
        # DETAILS
        # =========================================================
        details=_frame(main,fg=C["surface"],radius=18,bw=1,bc=C["border"])
        details.grid(row=3,column=0,sticky="ew",padx=24,pady=(4,12))
        details.grid_columnconfigure(0,weight=1)
        dh=ctk.CTkFrame(details,fg_color="transparent")
        dh.grid(row=0,column=0,sticky="ew",padx=18,pady=(16,8))
        dh.grid_columnconfigure(0,weight=1)
        _dw["title_lbl"]=_lbl(dh,"Centre de détails",C["text"],(FONT_HEAD,15,"bold"))
        _dw["title_lbl"].grid(row=0,column=0,sticky="w")
        _lbl(dh,"Sélectionnez un indicateur pour afficher des informations détaillées.",
            C["muted"],(FONT_BODY,11)).grid(row=1,column=0,sticky="w",pady=(3,0))
        dbs=_frame(details,fg=C["surface_2"],radius=12,bw=1,bc=C["border_soft"])
        dbs.grid(row=1,column=0,sticky="ew",padx=18,pady=(0,18))
        dbs.grid_columnconfigure(0,weight=1)
        _dw["body"]=ctk.CTkFrame(dbs,fg_color="transparent")
        _dw["body"].grid(row=0,column=0,sticky="ew",padx=16,pady=16)
        _render_detail()

        # =========================================================
        # ACTIONS
        # =========================================================
        act_hdr=_section_hdr(main,"Actions prioritaires",
                            "Accès direct aux parcours les plus utilisés de la plateforme.")
        act_hdr["wrap"].grid(row=4,column=0,sticky="ew",padx=24,pady=(4,4))
        act_cont=ctk.CTkFrame(main,fg_color="transparent")
        act_cont.grid(row=5,column=0,sticky="ew",padx=24,pady=(0,8))
        act_cards=[]

        def _action(parent,icon,title,desc,btn_text,cmd):
            nfg=C["surface_2"]; hfg=C["surface_3"]
            card=_frame(parent,fg=nfg,radius=16,bw=1,bc=C["border"])
            card.grid_columnconfigure(0,weight=1); _hover(card,nfg,hfg)
            tp=ctk.CTkFrame(card,fg_color=C["surface_4"],corner_radius=6,
                            border_width=1,border_color=C["border"])
            tp.grid(row=0,column=0,sticky="w",padx=18,pady=(18,10))
            pl=_lbl(tp,f"{icon}  Parcours",C["text_2"],(FONT_HEAD,10,"bold"))
            pl.pack(padx=10,pady=5)
            t=_lbl(card,title,C["text"],(FONT_HEAD,15,"bold"),justify="left",wraplength=340)
            t.grid(row=1,column=0,sticky="w",padx=18)
            d=_lbl(card,desc,C["text_2"],(FONT_BODY,12),justify="left",wraplength=360)
            d.grid(row=2,column=0,sticky="w",padx=18,pady=(10,16))
            b=_primary_btn(card,btn_text,cmd=cmd,h=40)
            b.grid(row=3,column=0,sticky="w",padx=18,pady=(0,18))
            ref={"card":card,"pill":pl,"title":t,"desc":d,"btn":b}
            act_cards.append(ref); return ref

        _action(act_cont,"◈","Visualisation réel vs prévisions",
                "Analysez un flux, une année et une filiale pour comparer les données observées aux trajectoires prévisionnelles.",
                "Ouvrir l'analyse",self.creer_page_graphique)
        _action(act_cont,"◉","Répartition des écarts par flux",
                "Identifiez les zones de tension et les flux les plus sensibles grâce à une lecture consolidée.",
                "Consulter la répartition",self.afficher_repartition_flux)
        _action(act_cont,"↓","Import des profils",
                "Chargez de nouveaux profils via un parcours simple et contrôlé pour sécuriser la qualité des données.",
                "Lancer l'import",self.importer_les_profils)

        # =========================================================
        # ACTIVITÉ RÉCENTE
        # =========================================================
        rec_hdr=_section_hdr(main,"Activité récente",
                            "Derniers fichiers détectés dans l'historique mensuel.")
        rec_hdr["wrap"].grid(row=6,column=0,sticky="ew",padx=24,pady=(8,4))
        rec_panel=_frame(main,fg=C["surface"],radius=18,bw=1,bc=C["border"])
        rec_panel.grid(row=7,column=0,sticky="ew",padx=24,pady=(0,24))
        rec_panel.grid_columnconfigure(0,weight=1)
        tbl=_frame(rec_panel,fg=C["surface_2"],radius=12,bw=1,bc=C["border_soft"])
        tbl.grid(row=0,column=0,sticky="ew",padx=16,pady=16)
        tbl.grid_columnconfigure(0,weight=5); tbl.grid_columnconfigure(1,weight=1)
        tbl.grid_columnconfigure(2,weight=2)

        def _th(col,text):
            l=_lbl(tbl,text,C["muted"],(FONT_HEAD,10,"bold"),anchor="w")
            l.grid(row=0,column=col,sticky="w",padx=14,pady=(12,6)); return l

        th_name=_th(0,"Fichier"); th_size=_th(1,"Taille"); th_date=_th(2,"Modifié le")
        ctk.CTkFrame(tbl,fg_color=C["border"],height=1,corner_radius=0).grid(
            row=1,column=0,columnspan=3,sticky="ew",padx=10,pady=(0,4))

        rec_rows=[]
        for idx,(nm,sz,dt) in enumerate(recent_files,start=2):
            rf=ctk.CTkFrame(tbl,fg_color=C["surface_2"],corner_radius=8)
            rf.grid(row=idx,column=0,columnspan=3,sticky="ew",padx=8,pady=3)
            rf.grid_columnconfigure(0,weight=5); rf.grid_columnconfigure(1,weight=1)
            rf.grid_columnconfigure(2,weight=2)
            _hover(rf,C["surface_2"],C["surface_3"])
            rn=_lbl(rf,nm,C["text"],  (FONT_BODY,12),anchor="w")
            rs=_lbl(rf,sz,C["text_2"],(FONT_BODY,12),anchor="w")
            rd=_lbl(rf,dt,C["text_2"],(FONT_BODY,12),anchor="w")
            rn.grid(row=0,column=0,sticky="w",padx=14,pady=10)
            rs.grid(row=0,column=1,sticky="w",padx=14,pady=10)
            rd.grid(row=0,column=2,sticky="w",padx=14,pady=10)
            rec_rows.append({"row":rf,"name":rn,"size":rs,"date":rd})

        # =========================================================
        # RESPONSIVE ENGINE
        # =========================================================
        def _regrid(cards,container,cols,gap):
            for i in range(max(cols,4)+1):
                try: container.grid_columnconfigure(i,weight=0)
                except: pass
            for i in range(cols):
                container.grid_columnconfigure(i,weight=1)
            for i,ref in enumerate(cards):
                try: ref["card"].grid_forget()
                except: pass
                ref["card"].grid(row=i//cols,column=i%cols,sticky="nsew",padx=gap,pady=gap)

        def _regrid_met(refs,cols,gap):
            for i in range(max(cols,4)+1):
                try: hero_met.grid_columnconfigure(i,weight=0)
                except: pass
            for i in range(cols): hero_met.grid_columnconfigure(i,weight=1)
            for i,ref in enumerate(refs):
                try: ref["card"].grid_forget()
                except: pass
                ref["card"].grid(row=i//cols,column=i%cols,sticky="ew",padx=gap,pady=gap)

        def _apply():
            if not (_exists(header) and _exists(sidebar) and _exists(main)): return
            r = _rs(); _rs_cache["v"] = r

            # ── Sidebar auto-collapse (sauf si l'utilisateur a toggle manuellement) ──
            auto = r["sb_auto"]
            cur  = _sb["mode"]
            if not _sb["manual"]:
                ...
                # Si fenêtre forcée trop petite, on force quand même icon
                if auto == "icon" and cur != "icon":
                    _sb["manual"] = False
                    _sb_set("icon")
                elif auto == "full" and cur == "icon":
                    # Fenêtre assez grande, on libère le verrou manuel
                    _sb["manual"] = False
                    _sb_set("full")
                else:
                    _sb_apply_content()
            ...
            hdr_title.configure(font=(FONT_HEAD,r["sec_fs"]-2,"bold"))
            hdr_sub.configure(
                text="Plateforme d'analyse des flux de trésorerie" if r["hdr_sub"] else "",
                font=(FONT_BODY,r["sm_fs"]))
            status_lbl.configure(
                text="● Opérationnel" if r["w"]<1180 else "● Système opérationnel",
                font=(FONT_HEAD,r["sm_fs"],"bold"))
            quit_btn.configure(height=r["btn_h"]-4,
                            width=88 if r["w"]>=980 else 72,
                            font=(FONT_HEAD,r["sm_fs"],"bold"))

            # ── Paddings ──
            for widget,px,py in [
                (hero,           r["pad"],(r["pad"],r["gap"])),
                (kpi_hdr["wrap"],r["pad"],(2,4)),
                (kpi_cont,       r["pad"],(0,r["gap"]-2)),
                (details,        r["pad"],(2,r["gap"])),
                (act_hdr["wrap"],r["pad"],(2,4)),
                (act_cont,       r["pad"],(0,r["gap"])),
                (rec_hdr["wrap"],r["pad"],(2,4)),
                (rec_panel,      r["pad"],(0,r["pad"])),
            ]:
                try: widget.grid_configure(padx=px,pady=py)
                except: pass

            # ── Hero ──
            left_hero.grid_configure(padx=r["inner"]+4,pady=r["inner"]+4)
            try: right_hero.grid_forget()
            except: pass
            hero.grid_columnconfigure(0,weight=1); hero.grid_columnconfigure(1,weight=0)
            if r["hero_stack"]:
                right_hero.grid(row=1,column=0,sticky="ew",
                                padx=r["inner"]+4,pady=(0,r["inner"]+4))
            else:
                right_hero.grid(row=0,column=1,sticky="ns",
                                padx=(0,r["inner"]+4),pady=r["inner"]+4)
            sum_card.configure(width=r["sum_w"] if not r["hero_stack"] and r["sum_w"] else 0)
            hero_pill_lbl.configure(font=(FONT_HEAD,r["sm_fs"],"bold"))
            hero_title.configure(font=(FONT_HEAD,r["hero_fs"],"bold"))
            hero_desc.configure(font=(FONT_BODY,r["hero_sub_fs"]),wraplength=r["hero_wrap"])

            for w_ in (hero_primary,hero_ghost):
                try: w_.pack_forget()
                except: pass
            ph=r["btn_h"]; pf=(FONT_HEAD,r["body_fs"],"bold")
            hero_primary.configure(height=ph,font=pf,width=220 if r["w"]>=1100 else 0)
            hero_ghost.configure(  height=ph,font=pf,width=188 if r["w"]>=1100 else 0)
            if r["btn_stack"]:
                hero_primary.pack(anchor="w",fill="x",pady=(0,8))
                hero_ghost.pack(  anchor="w",fill="x")
            else:
                hero_primary.pack(side="left",padx=(0,10))
                hero_ghost.pack(  side="left")

            _regrid_met(met_refs,r["met_cols"],r["gap"])
            for ref in met_refs:
                ref["lbl"].configure(font=(FONT_BODY,r["sm_fs"]))
                ref["val"].configure(font=(FONT_HEAD,r["sec_fs"]-2,"bold"),text_color=ref["accent"])
            for ref in mini_stats:
                ref["lbl"].configure(font=(FONT_BODY,r["sm_fs"]))
                ref["val"].configure(font=(FONT_HEAD,r["body_fs"]+1,"bold"),
                                    text_color=ref["accent"] or C["text"])

            # ── Section headers ──
            for sh in (kpi_hdr,act_hdr,rec_hdr):
                sh["title"].configure(font=(FONT_HEAD,r["sec_fs"],"bold"))
                if sh["subtitle"]: sh["subtitle"].configure(font=(FONT_BODY,r["sm_fs"]))

            # ── KPI ──
            _regrid(kpi_cards,kpi_cont,r["kpi_cols"],r["gap"])
            wl=420 if r["kpi_cols"]==1 else 300 if r["kpi_cols"]==2 else 260
            for ref in kpi_cards:
                ref["title"].configure(font=(FONT_BODY,r["card_fs"]))
                ref["val"].configure(  font=(FONT_HEAD,r["card_val_fs"],"bold"))
                ref["sub"].configure(  font=(FONT_BODY,r["sm_fs"]),wraplength=wl)
                if ref["btn"]:
                    ref["btn"].configure(height=max(28,r["btn_h"]-10),
                                        width=140 if r["w"]>=1100 else 120,
                                        font=(FONT_HEAD,r["sm_fs"],"bold"))

            # ── Detail ──
            if _exists(_dw["title_lbl"]):
                _dw["title_lbl"].configure(font=(FONT_HEAD,max(12,r["sec_fs"]-4),"bold"))
            _render_detail()

            # ── Actions ──
            _regrid(act_cards,act_cont,r["act_cols"],r["gap"])
            aw=480 if r["act_cols"]==1 else 360 if r["act_cols"]==2 else 300
            for ref in act_cards:
                ref["pill"].configure(font=(FONT_HEAD,r["sm_fs"],"bold"))
                ref["title"].configure(font=(FONT_HEAD,r["body_fs"]+3,"bold"),wraplength=aw)
                ref["desc"].configure( font=(FONT_BODY,r["body_fs"]),wraplength=aw+40)
                ref["btn"].configure(  height=r["btn_h"],font=(FONT_HEAD,r["body_fs"],"bold"),
                                    width=180 if r["w"]>=1100 else 155)

            # ── Table ──
            for lbl in (th_name,th_size,th_date):
                lbl.configure(font=(FONT_HEAD,r["sm_fs"],"bold"))
            for ref in rec_rows:
                ref["name"].configure(font=(FONT_BODY,r["body_fs"]))
                ref["size"].configure(font=(FONT_BODY,r["body_fs"]))
                ref["date"].configure(font=(FONT_BODY,r["body_fs"]))

        # =========================================================
        # BIND RESIZE
        # =========================================================
        def _schedule(_event=None):
            if _event is not None and _event.widget is not self: return
            _cancel_job("_accueil_resize_job")
            self._accueil_resize_job = self.after(55, _apply)

        self.bind("<Configure>", _schedule)
        self.after(130, _apply)
