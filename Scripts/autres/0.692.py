...
#===================Page Prédiction IA===================
    ...
    def _ia_build_prediction_page(self, df):
        ...
        def _train_model():
            """
            SA_VOYAGEURS – V2 (spikes)
            - Série métier (réel): WE/Fériés=0 + report prochain ouvré
            - Hurdle: classifier y>0 + reg log1p(y) (mieux sur pics)
            - Events: 22 + 22 effectif ouvré + near-22
            - Ajouts spikes: fenêtre 19–23, near-20, Monday, Monday_in_19_23
            - Ajouts calendrier: Last Business Day (LBD) + days_to_lbd
            - Validation: split année_target (val=base_year si possible)
            - Calibration: conformal P10/P90 par bins (niveau)
            """
            ...
            print("======================== TRAIN V2 (SA_VOYAGEURS – HURDLE + LOGREG + EVENT 22 + MIDMONTH + LBD + REPORT) ========================")
            ...
            try:
                ...
                if "is_holiday" not in df_filiale.columns:
                    ...
                    try:
                        ...
                        print(f"[HOLIDAYS] OK years={years}")
                    except Exception:
                ...
                def _last_business_day(year: int, month: int, holiday_set: set) -> pd.Timestamp:
                    # dernier jour calendaire du mois
                ...
                # ======================================================================
                # 2) Event tables (22 effectif, LBD) + features vectorisées
                # ======================================================================
                ...
                # --- Mid-month spikes (19–23) + near-20
                df_filiale["is_mid_month_window"] = df_filiale["dom"].between(19, 23).astype(int)
                ...
                df_filiale["is_monday_after_20"] = ((df_filiale["dow"] == 0) & (df_filiale["dom"].between(19, 23))).astype(int)
                ...
                df_filiale["abs_days_to_lbd"] = df_filiale["days_to_lbd"].abs().astype(int)
                ...
                s_prev = s_prev.rename(columns=prev_rename)
                ...
                # ======================================================================
                # 4) Split validation (année_target)
                # ======================================================================
                ...
                print(f"[TUNING] CLS best: {cls_bp} | CV AUC={cls_bs:.4f}")

                # ---- Regressor on log1p(y) for y>0
                ...
                print(f"[TUNING] REG(log) best: {reg_bp} | CV negRMSE(log)={reg_bs:.4f}")

                # ======================================================================
                # 6) Validation (scale originale) + calibration bins
                # ======================================================================
                p_valid = cls_model.predict_proba(X_valid)[:, 1]
                pred_log_valid = reg_model.predict(X_valid)
                amt_valid = np.expm1(np.clip(pred_log_valid, a_min=0.0, a_max=None))
                ...
                print(f"[MODEL V2] MAE={mae:.2f}, RMSE={rmse:.2f}, R²={r2:.3f}")
                print(f"[CALIB bins] Coverage={coverage:.3f}, Width={width:.1f}")
                ...
                _kpi(kpi_frame, "Event", "22 + 20-23 + LBD", "Spikes", 6)

                # ======================================================================
                # 7) Forecast N+1 (report + features target)
                # ======================================================================
                ...
                # build calmap target year (eff22 + lbd)
                calmap_tgt_rows = []
                for mm in range(1, 13):
                    try:
                ...
                def _calib_offsets(pred_value: float):
                    ...
                    return float(q_low_by_bin[b]), float(q_high_by_bin[b])
                ...
                for _, row in df_prev_year.iterrows():
                    ...
                    # --- target: mid-month window + near20 + monday
                    is_mid_window_tgt = 1 if (19 <= dom_tgt <= 23) else 0
                    abs_days_to_20_tgt = abs(dom_tgt - 20)
                    is_near_20_tgt = 1 if abs_days_to_20_tgt <= 1 else 0
                    is_monday_tgt = 1 if dow_next == 0 else 0
                    is_monday_after_20_tgt = 1 if (dow_next == 0 and 19 <= dom_tgt <= 23) else 0
                    ...
                    # --- prev-side already computed in df_prev_year
                    feat_df = pd.DataFrame([[
                        ...
                        # spikes prev
                        int(row.get("is_mid_month_window", 1 if 19 <= int(row["dom"]) <= 23 else 0)),
                        ...
                        int(row.get("is_monday_after_20", 1 if (int(row["dow"]) == 0 and 19 <= int(row["dom"]) <= 23) else 0)),
                    ...
                    pred_log = float(reg_model.predict(feat_df)[0])
                    ...
                    pred_p10 = max(0.0, pred_final + ql)
                    pred_p90 = max(0.0, pred_final + qh)
                ...
                # align to target real dates if present
                ...
                ax1.set_title("SA_VOYAGEURS – Prévision N+1 – V2 (log + mid-month + LBD + report)", color="white")
