...
def _safe_feature_importance(pipeline, feature_names):
    """
    Récupère une importance normalisée [0..1] si possible.
    CatBoost(wrapper): get_feature_importance
    LightGBM / arbres: feature_importances_
    Sinon: None
    """
    try:
    ...
    # CatBoost wrapper
    try:
        if hasattr(est, "_model") and est._model is not None:
            imp = np.array(est._model.get_feature_importance(), dtype=float)
...
def show_train_model_3d(model_name, feature_names, feature_importance=None):
    """
    Vue 3D PyVista fidèle à TON flow:
    Features -> StandardScaler -> Model(delta) -> Reconstruct level -> Weekend rule -> Clamp/Median -> Output
    """
    import pyvista as pv

    pv.set_plot_theme("dark")
    plotter = pv.Plotter(window_size=(1500, 880))
    ...
    for i, f in enumerate(feats):
        ...
        if feature_importance is not None and i < len(feature_importance):
            s = float(np.clip(feature_importance[i], 0, 1))
            sizes.append(0.35 + 2.4*s)
            cols.append(0.2 + 0.8*s)
        else:
            sizes.append(1.0)
            cols.append(0.8)

    # --- blocks
    def add_block(name, x, y, z, s=6.0, c=1.0):
        pts.append([x, y, z])
        labels.append(name)
        sizes.append(s)
        cols.append(c)

    add_block("StandardScaler", 7.0,  0.0,  3.0, s=6.0, c=1.05)
    add_block(f"Model Δ\n{model_name}", 13.0, 0.0, 0.0, s=7.0, c=1.15)
    add_block("Reconstruct level\n(y_prev + Δ)", 18.0, 0.0, 0.0, s=6.5, c=1.05)
    add_block("Rule\nweekend=0\nreport Monday", 23.0, 2.7, 0.0, s=6.0, c=0.95)
    add_block("Post\nclamp + median", 23.0, -2.7, 0.0, s=6.0, c=0.95)
    add_block("Output\npred_value", 30.0, 0.0, 0.0, s=7.5, c=1.25)
    ...
    for i in range(nF):
        src.append(i); dst.append(idx_scaler)

    src += [idx_scaler, idx_model, idx_recon]
    dst += [idx_model,  idx_recon, idx_out]

    # branches rule/post vers output
    src += [idx_recon, idx_recon, idx_rule, idx_post]
    dst += [idx_rule,  idx_post,  idx_out,  idx_out]
    ...
    def build_edges_pointdata(src, dst, w):
        e = len(src)
        pe = np.empty((2*e, 3), dtype=np.float32)
        pe[0::2] = pts[src]
        pe[1::2] = pts[dst]
    ...
    sphere = pv.Sphere(radius=0.55, theta_resolution=18, phi_resolution=18)
    glyphs = cloud.glyph(scale="s", geom=sphere, orient=False)

    edge_mesh = build_edges_pointdata(src, dst, w)
    edge_actor = plotter.add_mesh(edge_mesh, scalars="w", cmap="turbo",
                                  opacity=0.40, line_width=3, lighting=False)

    plotter.add_mesh(glyphs, scalars="c", cmap="turbo",
                     opacity=0.98, smooth_shading=True, specular=0.65)

    plotter.add_text("PULSE — MODEL VIEW 3D", position="upper_left", font_size=12, color="white")
    plotter.add_text("Auto-rotate + pulsating edges", position="lower_left", font_size=11, color="white")

    # labels features si pas trop
    if nF <= 35:
        plotter.add_point_labels(pts[:nF], labels[:nF], font_size=10,
                                 text_color="white", shape_opacity=0.0)

    plotter.add_point_labels(pts[nF:], labels[nF:], font_size=14,
                             text_color="white", shape_opacity=0.15)

    state = {"t": 0.0}

    def tick():
        state["t"] += 0.08
        w[:] = 0.20 + 0.80*(0.5 + 0.5*np.sin(state["t"]))
        new_edge = build_edges_pointdata(src, dst, w)
        edge_actor.mapper.dataset.shallow_copy(new_edge)
    ...
    plotter.iren.create_repeating_timer(35)
    plotter.iren.start()


def open_train_model_3d_thread(model_name, feature_names, feature_importance=None):
    threading.Thread(
        target=lambda: show_train_model_3d(model_name, feature_names, feature_importance),
        daemon=True
    ).start()
    ...
    def _ia_build_prediction_page(self, df):
        ...
        class CatBoostSklearnRegressor(BaseEstimator, RegressorMixin):
            ...
            def fit(self, X, y):
                ...
                self.model_ = CatBoostRegressor(**self.kwargs)
        ...
        def _train_model():
            """
            TRAIN NN TABULAIRE ULTRA (ResMLP) — version robuste avec peu d'années
            - cible = delta (y_target - y_prev_year)
            - features calendrier (DOY sin/cos, ISO week sin/cos, dow, month, dom, eom, weekend, holiday, bridge)
            - lags 1/7/14/28/30/90 + rolling log
            - TimeSeriesSplit : métriques sur NIVEAU reconstruit
            - garde meilleur fold (RMSE min) + refit cumulative
            - prédiction N+1 vectorisée
            - règle métier : week-end=0 + report lundi
            - affichage KPI + graph + analyse
            """
            nonlocal graph_widgets
            nonlocal current_pred_df, current_real_target_df, current_target_year, current_filiale_name
            ...
            print("===== TRAIN NN TABULAIRE (DELTA + CALENDAR + TS BEST + WEEKEND=0) =====")

            # ----------- Filiale/année depuis UI (PAS hardcodé) -----------
            filiale = selected_filiale.get() if "selected_filiale" in locals() else "SA_VOYAGEURS"
            ...
            try:
                ...
                # ====== LAGS / LOG / ROLLINGS par année ======
                for lag in (1, 7, 14, 28, 30, 90):
                    df_filiale[f"lag_{lag}"] = df_filiale.groupby("year")["y"].shift(lag)

                df_filiale["y"] = pd.to_numeric(df_filiale["y"], errors="coerce").fillna(0.0)
                df_filiale["y_log"] = np.log1p(df_filiale["y"].clip(lower=0))
                ...
                for lag in (1, 7, 14, 28, 30, 90):
                    df_filiale[f"lag_{lag}_log"] = np.log1p(df_filiale[f"lag_{lag}"].fillna(0).clip(lower=0))

                # ====== PAIRS N -> N+1 via dayofyear ======
                s = df_filiale.copy()
                s_prev = s.copy()
                ...
                merged = pd.merge(
                    ...
                    how="inner",
                ).rename(columns={"y": "y_target"})

                if merged.empty:
                    messagebox.showinfo("Information", f"Aucune paire année→année pour {filiale}.")
                    return

                merged["delta_target"] = merged["y_target"] - merged["y_prev_year"]

                df_pairs = merged[[
                    "year_target", "dayofyear",
                    "y_prev_year", "y_target", "delta_target",

                    "y_prev_year_log", "roll_prev_7_log", "roll_prev_30_log", "roll_prev_90_log",
                    "lag_1_prev_log", "lag_7_prev_log", "lag_14_prev_log", "lag_28_prev_log", "lag_30_prev_log", "lag_90_prev_log",

                    "doy_sin_prev", "doy_cos_prev",
                    "iso_week_prev", "woy_sin_prev", "woy_cos_prev",

                    "dow_prev", "month_prev", "dom_prev", "is_eom_prev",
                    "is_weekend_prev", "is_holiday_prev", "is_bridge_prev",
                ]].copy()

                # ====== TRAIN <= base_year ======
                df_train_pairs = df_pairs[df_pairs["year_target"] <= base_year].copy()
                if len(df_train_pairs) < 200:
                    messagebox.showinfo("Information", f"Pas assez de données (pairs) pour entraîner jusqu'à {base_year}.")
                    return
                ...
                df_train_pairs = df_train_pairs.dropna(subset=features + ["delta_target", "y_target", "y_prev_year"]).reset_index(drop=True)

                X = df_train_pairs[features].replace([np.inf, -np.inf], np.nan).fillna(0.0).astype("float32")
                y_delta = df_train_pairs["delta_target"].replace([np.inf, -np.inf], np.nan).fillna(0.0).astype("float32")

                if len(X) < 200:
                    messagebox.showinfo("Information", "Après dropna, pas assez de données.")
                    return

                X_np = X.values
                y_np = y_delta.values

                # ====== Normalisation stable (très important pour NN) ======
                X_mean = X_np.mean(axis=0, keepdims=True)
                X_std  = X_np.std(axis=0, keepdims=True) + 1e-6
                Xn = (X_np - X_mean) / X_std

                # ====== Modèle NN tabulaire deep (ResMLP) ======
                ...
                class TabularResMLP(nn.Module):
                    def __init__(self, d_in, width=512, depth=10, p=0.15):
                        super().__init__()
                ...
                model = TabularResMLP(d_in=Xn.shape[1], width=512, depth=12, p=0.15).to(device)
                model_name = "TabularResMLP(512x12)"
                ...
                loss_fn = nn.SmoothL1Loss(beta=500.0)  # Huber stable sur gros montants

                # ====== TimeSeriesSplit + best fold sur NIVEAU ======
                ...
                X_torch = torch.from_numpy(Xn)
                y_torch = torch.from_numpy(y_np)

                for fold, (tr, va) in enumerate(tscv.split(Xn), start=1):
                    # fresh init each fold
                    model = TabularResMLP(d_in=Xn.shape[1], width=512, depth=12, p=0.15).to(device)
                    opt = torch.optim.AdamW(model.parameters(), lr=2e-4, weight_decay=1e-2)
                    scaler = GradScaler(enabled=(device.type == "cuda"))

                    ds_tr = TensorDataset(X_torch[tr], y_torch[tr])
                    ...
                    for ep in range(epochs):
                        ...
                        # mini early stop “soft”
                        if ep in (0, 10, 20, 40, 60):
                            pass

                    # eval fold sur NIVEAU reconstruit
                    model.eval()
                    with torch.no_grad():
                        va_pred_delta = model(X_torch[va].to(device)).detach().cpu().numpy()

                    y_prev = df_train_pairs.iloc[va]["y_prev_year"].values.astype(float)
                    y_true = df_train_pairs.iloc[va]["y_target"].values.astype(float)
                    y_pred = y_prev + va_pred_delta

                    rmse_f = float(np.sqrt(np.mean((y_true - y_pred) ** 2)))
                    mae_f  = float(np.mean(np.abs(y_true - y_pred)))
                    r2_f   = float(r2_score(y_true, y_pred))

                    print(f"[TS Fold {fold}] model={model_name} RMSE(level)={rmse_f:.1f} MAE={mae_f:.1f} R2={r2_f:.3f}")
                    fold_rows.append((fold, r2_f, rmse_f, mae_f, tr, va))
                ...
                print(f"[TS mean] model={model_name} R2={r2_mean:.3f} RMSE={rmse_mean:.1f} MAE={mae_mean:.1f}")
                print(f"[TS BEST] fold={best_fold} RMSE={best_rmse:.1f} refit_up_to={best_end_idx}")

                # ====== Refit cumulative 0..best_end_idx ======
                model = TabularResMLP(d_in=Xn.shape[1], width=512, depth=12, p=0.15).to(device)
                model.load_state_dict(best_state)
                model.train()

                tr_idx = np.arange(0, best_end_idx + 1)
                ds_tr = TensorDataset(X_torch[tr_idx], y_torch[tr_idx])
                dl_tr = DataLoader(ds_tr, batch_size=batch_size, shuffle=True, drop_last=False)
                ...
                df_prev_year = df_prev_year.sort_values("date").reset_index(drop=True)
                df_prev_year["y"] = pd.to_numeric(df_prev_year["y"], errors="coerce").fillna(0.0)

                for lag in (1, 7, 14, 28, 30, 90):
                    df_prev_year[f"lag_{lag}"] = df_prev_year["y"].shift(lag)

                df_prev_year["y_prev_year_log"] = np.log1p(df_prev_year["y"].clip(lower=0))
                df_prev_year["roll_prev_7_log"]  = np.log1p(df_prev_year["y"].clip(lower=0).rolling(7,  min_periods=1).mean())
                df_prev_year["roll_prev_30_log"] = np.log1p(df_prev_year["y"].clip(lower=0).rolling(30, min_periods=1).mean())
                df_prev_year["roll_prev_90_log"] = np.log1p(df_prev_year["y"].clip(lower=0).rolling(90, min_periods=1).mean())

                for lag in (1, 7, 14, 28, 30, 90):
                    df_prev_year[f"lag_{lag}_prev_log"] = np.log1p(df_prev_year[f"lag_{lag}"].fillna(0).clip(lower=0))

                future_dates = pd.to_datetime(df_prev_year["date"] + pd.DateOffset(years=1))
                df_future_feat = _add_calendar_features(pd.DataFrame({"date": future_dates}), years_for_holidays=years_for_holidays)

                X_future = pd.DataFrame({
                    "dayofyear": df_future_feat["dayofyear"].astype(int),

                    "doy_sin_prev": df_future_feat["doy_sin"].astype(float),
                    "doy_cos_prev": df_future_feat["doy_cos"].astype(float),

                    "iso_week_prev": df_future_feat["iso_week"].astype(int),
                    "woy_sin_prev": df_future_feat["woy_sin"].astype(float),
                    "woy_cos_prev": df_future_feat["woy_cos"].astype(float),

                    "dow_prev": df_future_feat["dow"].astype(int),
                    "month_prev": df_future_feat["month"].astype(int),
                    "dom_prev": df_future_feat["dom"].astype(int),
                    "is_eom_prev": df_future_feat["is_eom"].astype(int),

                    "is_weekend_prev": df_future_feat["is_weekend"].astype(int),
                    "is_holiday_prev": df_future_feat["is_holiday"].astype(int),
                    "is_bridge_prev": df_future_feat["is_bridge"].astype(int),

                    "y_prev_year_log": df_prev_year["y_prev_year_log"].astype(float),
                    "roll_prev_7_log": df_prev_year["roll_prev_7_log"].astype(float),
                    "roll_prev_30_log": df_prev_year["roll_prev_30_log"].astype(float),
                    "roll_prev_90_log": df_prev_year["roll_prev_90_log"].astype(float),

                    "lag_1_prev_log": df_prev_year["lag_1_prev_log"].astype(float),
                    "lag_7_prev_log": df_prev_year["lag_7_prev_log"].astype(float),
                    "lag_14_prev_log": df_prev_year["lag_14_prev_log"].astype(float),
                    "lag_28_prev_log": df_prev_year["lag_28_prev_log"].astype(float),
                    "lag_30_prev_log": df_prev_year["lag_30_prev_log"].astype(float),
                    "lag_90_prev_log": df_prev_year["lag_90_prev_log"].astype(float),
                })

                Xf = X_future[features].replace([np.inf, -np.inf], np.nan).fillna(0.0).astype("float32").values
                Xf = (Xf - X_mean) / X_std

                model.eval()
                with torch.no_grad():
                    delta_pred = model(torch.from_numpy(Xf).to(device)).detach().cpu().numpy()

                y_prev = df_prev_year["y"].astype(float).values
                y_pred_raw = np.maximum(y_prev + delta_pred, 0.0)

                df_future_all = pd.DataFrame({
                    "section": filiale,
                    "date": future_dates,
                    "year": future_dates.dt.year.astype(int),
                    "month": future_dates.dt.month.astype(int),
                    "dayofyear": future_dates.dt.dayofyear.astype(int),
                    "pred_value": y_pred_raw.astype(float)
                }).sort_values("date").drop_duplicates(subset=["date"], keep="first").reset_index(drop=True)
                ...
                df_future_all = df_future_all.drop(columns=["dow"])

                # ====== KPI UI (affiche bien le modèle) ======
                ...
                def _kpi(parent, title, value, subtitle, col):
                    ...
                    ctk.CTkLabel(card, text=title, font=("Segoe UI", 12), text_color="#9fb7dd")\
                        .grid(row=0, column=0, sticky="w", padx=14, pady=(12, 0))
                    ctk.CTkLabel(card, text=value, font=("Segoe UI Semibold", 18, "bold"), text_color="white")\
                ...
                _kpi(kpi_frame, "MAE (mean)", f"{mae_mean:.0f}", f"R² mean={r2_mean:.3f}", 1)
                _kpi(kpi_frame, "Modèle", model_name, f"params={n_params:,} | device={device.type}", 2)
                ...
                ax1.plot(df_future_all["date"], df_future_all["pred_value"],
                        label=f"Prévision NN {target_year} ({model_name})",
                        linewidth=2, linestyle="--", color="#ff7f0e")

                ax1.set_title(f"{filiale} – Prévision N+1 (NN Tabulaire)", color="white")
