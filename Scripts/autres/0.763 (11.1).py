...
#===================Page Prédiction IA===================
    ...
    def _ia_build_prediction_page(self, df):
        ...
        # ===================== 1) _compute_metrics : version complète mise à jour =====================
        def _compute_metrics(
            ...
            eps=1e-9,
            # --- NEW ---
            exclude_above_k=None,     # ex: 100000 pour 100 M€
            restrict_dates=None,
        ):
            """
            Analyse quant robuste :
            - filtre métier |Réel| >= min_abs_ref
            - filtre optionnel jours > seuil fixe (ex: 100 000)
            - possibilité de restreindre à une période donnée
            """
            ...
            # Restriction période spécifique (ex: profil)
        ...
        def _train_model():
            ...
            + AJOUT : offset additif (median residual) appris sur VALID (clamp) et appliqué aussi en forecast.

            Objectif: enlever le petit biais résiduel sans casser les alertes/pics.
            """
            ...
            print("======================== TRAIN V3.3 (SA_VOYAGEURS – V3.2 + OFFSET median residual) ========================")
            ...
            try:
                ...
                # spike_risk (gate large mais window resserrée)
                ...
                # ======================================================================
                # 8) Scale anti-biais (VALID) + OFFSET (median residual)
                # ======================================================================
                ...
                offset = max(-8000.0, min(offset, 8000.0))  # clamp safe
                ...
                # ======================================================================
                # 9) Validation + calibration bins=7 (sur prédictions finales VALID)
                # ======================================================================
                ...
                # ======================================================================
                # 10) Forecast N+1 (report + scale + offset) + bande
                # ======================================================================
                ...
                def _bin_idx(v: float) -> int:
                    return int(np.digitize([v], qs[1:-1], right=True)[0])

                def _band(pred_value: float):
                    b = _bin_idx(pred_value)
                ...
                for _, row in df_prev_year.iterrows():
                    ...
                    # scale AFTER report + offset (NEW)
                    pred_final = pred_final * scale + offset
                    if pred_final < 0:
                        pred_final = 0.0

                    ql, qh = _band(pred_final)
                ...
                ax1.fill_between(df_future_all["date"], df_future_all["pred_p10"], df_future_all["pred_p90"],
                                alpha=0.25, label=f"Bande calibrée (bins={n_bins}) P10–P90")

                ax1.set_title("SA_VOYAGEURS – Prévision N+1 – V3.3 (window 20-22 + q75 + offset)", color="white")
