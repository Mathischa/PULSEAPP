...
#===================Page Prédiction IA===================
    ...
    def _ia_build_prediction_page(self, df):
        ...
        def _train_model():
            """
            ===== TRAIN NN TABULAIRE (MODE 2 = PERF MAX / DÉMO) =====
            - Utilise TOUTES les paires année→année disponibles (ex: 2023→2024, 2024→2025, 2025→2026)
            même si ça "regarde le futur" (c’est le mode démo/perf).
            - Alignement robuste bissextile : merge sur clé "MM-DD" et exclusion du 29/02.
            - Target = log-growth clampé :
                g = log1p(y_target) - log1p(y_prev)
            et reconstruction niveau :
                y_pred = expm1(log1p(y_prev) + g_pred)
            - Dataset augmenté via fenêtres glissantes (K jours) sur features tabulaires :
                X_i = [feat_{i-K+1} ... feat_i] (flatten)
                y_i = g_i
            - TimeSeriesSplit sur l’ordre temporel (trié par date_target).
            Sélection du meilleur fold (RMSE niveau min) + refit cumulatif 0..fin(best fold).
            - Prédiction N+1 (base_year→target_year) sur 365 jours (sans 29/02),
            + règle métier week-end=0 + report lundi.
            - Affiche clairement le modèle et ses params + KPI.
            - Vue 3D Matplotlib live (optionnelle, essaie de s’ouvrir).
            """
            nonlocal graph_widgets
            nonlocal current_pred_df, current_real_target_df, current_target_year, current_filiale_name
            ...
            # ----------------------------
            # Paramètres (tu peux tweak)
            # ----------------------------
            filiale = "SA_VOYAGEURS"
            current_filiale_name = filiale

            K = 30                 # fenêtre glissante (jours)
            gmax = 0.8             # clamp log-growth (stabilise)
            n_splits = 5
            ...
            epochs = 120
            batch_size = 128
            patience = 18          # early stop par fold

            # ----------------------------
            # 3D View (optionnelle)
            # ----------------------------
            def launch_3d_view(model: nn.Module):
                """
                Fenêtre Tk + Matplotlib 3D live.
                Visual "impressionnant" : nœuds par couche + connexions,
                l'épaisseur bouge avec l'énergie des poids.
                """
                from mpl_toolkits.mplot3d import Axes3D  # noqa

                win = tk.Toplevel(self)
                win.title("Vue 3D - Réseau (live)")
                ...
                for li in range(L-1):
                    for a in range(per_layer):
                        ...
                        b2 = (a*3 + 7) % per_layer
                        i = li*per_layer + a
                        j1 = (li+1)*per_layer + b1
                        j2 = (li+1)*per_layer + b2
                        edges.append((i, j1))
                        edges.append((i, j2))

                def _weight_energy(m):
                    vals = []
                    with torch.no_grad():
                        for p in m.parameters():
                            if p is None or (not p.requires_grad):
                                continue
                            if p.ndim >= 2 and p.numel() > 2000:
                                vals.append(float(p.detach().float().norm().cpu().item()))
                    if not vals:
                        return 1.0
                ...
                def redraw():
                    ...
                    e = _weight_energy(model)
                    base = 0.25 + 0.05*np.log1p(e)
                    pulse = 0.5 + 0.5*np.sin(state["t"])
                    lw = base * (0.9 + 0.9*pulse)

                    ax.view_init(
                        elev=22 + 10*np.sin(state["t"]*0.7),
                        azim=25 + 40*np.sin(state["t"]*0.35)
                    )
            ...
            # ----------------------------
            # Modèle : MLP résiduel tabulaire (flatten K*F -> g)
            # ----------------------------
            ...
            class TabularResMLP(nn.Module):
                def __init__(self, in_dim, hidden=384, blocks=8, drop=0.15, gmax=0.8):
                    super().__init__()
            ...
            try:
                ...
                # Features calendrier + logs/rollings/lags par année
                ...
                # features tabulaires (une ligne par jour, sur la base du "prev year" + calendrier target)
                # On fusionne sur md, mais on garde date_target pour tri TS.
                base_cols_prev = [
                ...
                # Calendrier TARGET (sur date_target) : on recalcule proprement
                ...
                # ----------------------------
                # Fenêtres glissantes par année_target (K jours)
                # ----------------------------
                X_list, y_list = [], []
                ...
                for yT, gdf in merged.groupby("year_target", sort=True):
                    ...
                    # pad gauche pour les 1ers jours
                    pad = np.repeat(Xday[:1], repeats=max(0, K-1), axis=0)
                    Xpad = np.vstack([pad, Xday])  # (K-1 + nD, F)

                    for i in range(nD):
                        w = Xpad[i:i+K]                 # (K,F)
                        X_list.append(w.reshape(-1))    # (K*F,)
                ...
                if len(X_list) < 500:
                    print("[WARN] Peu d'échantillons après windowing. Tu auras un modèle instable. (Mais ça tournera.)")
                ...
                # Tri temporel global (important pour TimeSeriesSplit)
                ...
                model0 = TabularResMLP(in_dim=in_dim, hidden=hidden, blocks=blocks, drop=dropout, gmax=gmax).to(device)
                params = sum(p.numel() for p in model0.parameters())
                print(f"[MODEL] {model_name} | params={params:,} | device={device.type}")

                # Vue 3D (best effort)
                try:
                    _view3d = launch_3d_view(model0)
                except Exception as e:
                    print("[WARN] Vue 3D non lancée:", e)
                    _view3d = None

                # ----------------------------
                # Utilitaires : scaler numpy (fit sur fold train)
                # ----------------------------
                def fit_scaler(Xtr):
                ...
                # Train fold
                # ----------------------------
                def train_one_fold(Xtr, ytr, Xva, yva, yprev_va, ytrue_va):
                    mu, sd = fit_scaler(Xtr)
                    Xtr_s = apply_scaler(Xtr, mu, sd)
                    Xva_s = apply_scaler(Xva, mu, sd)

                    m = TabularResMLP(in_dim=in_dim, hidden=hidden, blocks=blocks, drop=dropout, gmax=gmax).to(device)
                    ...
                    for ep in range(1, epochs + 1):
                        ...
                        for xb, yb in dl_tr:
                            ...
                            pred = m(xb)
                            loss = F.smooth_l1_loss(pred, yb)  # robuste
                            loss.backward()
                        ...
                        r2 = float(1.0 - (np.sum((ytrue_va - yhat)**2) / (np.sum((ytrue_va - np.mean(ytrue_va))**2) + 1e-9)))
                        ...
                        if ep % 10 == 0 or ep == 1:
                            print(f"[EP {ep:03d}] train_loss={loss_sum/max(nb,1):.4f} | val RMSE(level)={rmse:.1f} MAE={mae:.1f} R2={r2:.3f} | bestRMSE={best_rmse:.1f}")
                ...
                # ----------------------------
                # TimeSeriesSplit + best fold (RMSE niveau)
                # ----------------------------
                ...
                for fold, (tr, va) in enumerate(tscv.split(X_np), start=1):
                    ...
                    m, mu, sd, rmse = train_one_fold(Xtr, ytr, Xva, yva, yprev_va, ytrue_va)

                    # métriques fold (avec modèle m)
                    Xva_s = apply_scaler(Xva, mu, sd)
                    with torch.no_grad():
                        m.eval()
                    ...
                    fold_rows.append((fold, r2_f, rmse_f, mae_f, int(np.max(va)), mu, sd, m.state_dict()))
                ...
                model_final = TabularResMLP(in_dim=in_dim, hidden=hidden, blocks=blocks, drop=dropout, gmax=gmax).to(device)
                ...
                for ep in range(1, epochs + 1):
                    ...
                    if ep % 20 == 0 or ep == 1:
                        print(f"[REFIT EP {ep:03d}] loss={loss_m:.4f} | best={best_loss:.4f}")
                ...
                # Align md prev-year <-> future-year
                ...
                # Build day feature rows (mêmes feat_cols que train)
                ...
                # ----------------------------
                # MAJ global + analyse + graph2/monthly
                # ----------------------------
